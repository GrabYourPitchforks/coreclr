...
namespace System
{
    ...
    public sealed partial class String : IComparable, IEnumerable, IConvertible, IEnumerable<char>, IComparable<string>, IEquatable<string>, ICloneable
    {
        ...
        public UnicodeScalar GetScalarAt(int index)
        {
            ...
            IntPtr ip = (IntPtr)index;

            uint firstChar = Unsafe.Add(ref GetRawStringData(), ip);
            if (!UnicodeHelpers.IsSurrogateCodePoint(firstChar))
            {
                // Common case: a BMP character that's not a surrogate
                return UnicodeScalar.DangerousCreateWithoutValidation(firstChar);
            }
            ...
            // We can read past the end of the string without performing an additional
            // length check because we know that strings are null-terminated. It's dangerous
            // but supported.

            uint secondChar = Unsafe.Add(ref Unsafe.Add(ref GetRawStringData(), ip), 1);

            if (!UnicodeHelpers.IsLowSurrogateCodePoint(secondChar))
            {
                // TODO: Return a better error message.
                ThrowHelper.ThrowArgumentOutOfRange_IndexException();
            }

            return UnicodeScalar.DangerousCreateWithoutValidation(UnicodeHelpers.GetScalarFromUtf16SurrogatePair(firstChar, secondChar));
        }

        public bool TryGetScalarAt(int index, out UnicodeScalar value)
        {
            if ((uint)index >= (uint)Length)
            {
                ThrowHelper.ThrowArgumentOutOfRange_IndexException();
            ...
            {
                // Common case: a BMP character that's not a surrogate
                value = UnicodeScalar.DangerousCreateWithoutValidation(firstChar);
                return true;
            }
            ...
            value = UnicodeScalar.DangerousCreateWithoutValidation(UnicodeHelpers.GetScalarFromUtf16SurrogatePair(firstChar, secondChar));
            return true;
